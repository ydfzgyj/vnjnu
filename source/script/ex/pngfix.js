var DD_belatedPNG={
	...
	createVmlNameSpace:function(){
		...
		if(window.attachEvent){window.attachEvent('onbeforeunload',function(){DD_belatedPNG=null;});}
	},
	...
	readPropertyChange:function(){
		...
		if(event.propertyName=='style.display'){
			...
			for(var v in el.vml){el.vml[v].shape.style.display=display;}
		}
	...
	handlePseudoHover:function(el){setTimeout(function(){DD_belatedPNG.applyVML(el);},1);},
	fix:function(selector){
		var selectors=selector.split(',');
		for(var i=0;i<selectors.length;i++){this.styleSheet.addRule(selectors[i],'behavior:expression(DD_belatedPNG.fixPng(this))');}
	},
	applyVML:function(el){
		...
		this.vmlOffsets(el);
		this.vmlOpacity(el);
		if(el.isImg){this.copyImageBorders(el);}
	},
	attachHandlers:function(el){
		...
		if(el.nodeName=='A'){
			...
			for(var a in moreForAs){handlers[a]=moreForAs[a];}
		}
	...
	copyImageBorders:function(el){
		...
		for(var s in styles){el.vml.color.shape.style[s]=el.currentStyle[s];}
	},
	vmlFill:function(el){
		if(!el.currentStyle){return;}else{var elStyle=el.currentStyle;}
		...
		var figurePercentage=function(axis,position){
			...
			switch(position){
				...
				case 'bottom':
					...
					break;
				default:
					if(position.search('%')!=-1){bg[axis]=parseInt(position)*.01;}else{fraction=false;}
			}
			var horz=(axis=='X');
			bg[axis]=Math.ceil(fraction?((size[horz?'W':'H']*bg[axis])-(size[horz?'w':'h']*bg[axis])):parseInt(position));
			if(bg[axis]==0){bg[axis]++;}
		};
		for (var b in bg){figurePercentage(b,thisStyle['backgroundPosition'+b]);}
	...
	fixPng:function(el){
		...
		if(el.nodeName=='BODY'||el.nodeName =='TD'||el.nodeName=='TR'){return;}
		el.isImg=false;
		if(el.nodeName=='IMG'){
